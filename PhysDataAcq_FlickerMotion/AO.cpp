...
void AO( MenuReturnValues mValues, int idx )
{
	...
	vector<double>	Y;
	//ADDED IN X-SIGNAL AND Y-SIGNAL INITIALIZATION 10/30/2017 [AWT]
	//-------------------------------------------------------------------------------------------------
	...
	/*********************************************/
	NZSample = ConstructAOBuffer_uInt16(&dAOBuffer[0],AOOneChanBufSiz,AOFileSize,NZSample, ptrAOFile, ptrAO_X_File, ptrAO_Y_File, NRepeats, TotNZSample); //4 arguments to 9 arguments 10/30/2017 [AWT]
	...
	while ( !done && !_kbhit() && NHalfBufs < (AOFileSize/AOOneChanBufSiz*NRepeats) ) //kbhit() => _kbhit() 10/18/2017 [AWT]
	{
		//********************************************
		...
		if(AOBufferSpaceFree > AOOneChanBufSiz) {  // Made this change and need to check. Switched AOHalfBuf_Siz for AOOneChanBufSiz
			++NHalfBufs;
			NZSample = ConstructAOBuffer_uInt16(&dAOBuffer[0],AOOneChanBufSiz,NZSample,AOFileSize,ptrAOFile,ptrAO_X_File,ptrAO_Y_File,NRepeats,TotNZSample); //4 arguments to 9 arguments 10/30/2017 [AWT]
