...
void AOContDITimeStamp_Int16( MenuReturnValues mValues, int idx )
{
	...
	if (SingleChannel == 1)
		sprintf_s(PhysicalAOChannels, "/Dev2/ao%i", NumAOChannels-1); //changed sprintf() to sprintf_s() 10/16/2017 [AWT]
	else if (SingleChannel == 0)
		sprintf_s(PhysicalAOChannels, "/Dev2/ao0:%i", NumAOChannels-1); //changed sprintf() to sprintf_s() 10/16/2017 [AWT]
	else {
	...
	/*********************************************/
	char fileIndex[65];
	_itoa_s(idx,fileIndex,10); //changed itoa() to _itoa_s() 10/16/2017 [AWT]
	...
	while ( !done && !_kbhit() && NHalfBufs < TotalHalfBufs ) //changed kbhit() to _kbhit() 10/16/2017 [AWT]
	{
		//********************************************
