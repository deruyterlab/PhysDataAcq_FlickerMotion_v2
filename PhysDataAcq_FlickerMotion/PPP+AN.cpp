...
void PPP_AN(MenuReturnValues mValues, int idx)
{
	bool32		done = 0;
	char		errBuff[2048] = { '\0' };
	int32		error = 0;
	...
	// Temporary definitions of parameters to be defined later by the user  		
	...
	//---------------------------------------------
	// Loading Sampling Array Yaw Positions.
	...
	//---------------------------------------------
	// Loading Sampling Array Pitch Positions.
	...
	//cout << "16. Loading Contrast3_AN" << endl; //[12/5/2017]
	//---------------------------------------------
	// Loading contrast3 for 3rd image
	//string strAO_Cont3;
	//ifstream* ptrCont3 = 0;
	//uInt32 filesize_Cont3;
	//vector<float> cont3;

	//strAO_Cont3 = mValues.strStimFileDirPath + "\\" + "Cont3.dat";
	//ptrCont3 = fnOpenFileToRead(strAO_Cont3, &filesize_Cont3);
	//cont3.assign(filesize_Cont3, 0);
	//for (uInt32 idx2 = 0; idx2<filesize_Cont3; idx2++) {
		//	ptrCont3->read(reinterpret_cast<char *>(&outputBuffer), sizeof(float));
	//	cont3[idx2] = outputBuffer;
	//}
	//ptrCont3->close();
	...
	/*****************************************************************/
	int16	nTurns = (int16)(filesize_TimeLag);		// # of times the stimulus changes in a trial
	...
	//cout << "---------------------------------------------" << endl;
	//for (int i=0; i<Pixels*nTurns; i++){
	//	cout <<"i="<<i<<"\tLoc2["<<i<<"]="<<Loc2[i]<<"\tLoc3["<<i<<"]="<<Loc3[i]<<endl;
	//}
	...
	TotalHalfBufs = ((filesize_YawPos / NFramesPerHalfBufer) * NRepeats);   // This part does not match with "AOAI_TimeStamp_RTInterp.cpp"
