...
void AOContDITimeStamp( MenuReturnValues mValues, int idx )
{
	...
	uInt32		AOOneChanBufSiz	= 1000000;
	...
	ifstream*	ptrAO_X_File	= 0; // changed from ifstream* => double* to agree with ConstructAOBuffer() [AWT] [4/30/2018] -- changed back, messed up another value assignment in this file
	uInt32		AO_X_FileSize	= 0; 
	string		strAO_X_FileName;

	ifstream*	ptrAO_Y_File	= 0; // changed from ifstream* => double* to agree with ConstructAOBuffer() [AWT] [4/30/2018] -- changed back, messed up another value assignment in this file
	...
	/*********************************************/
	char fileIndex[3];
	_itoa_s(idx,fileIndex,5); //itoa() => _itoa_s() 10/18/2017 [AWT]
	...
	/*********************************************/	
	// Initialize the Stimulus file (X,Y-signal)
	/*********************************************/
	...
	ptrAO_Y_File = fnOpenFileToRead(strAO_Y_FileName, &AO_Y_FileSize); //Call function to open Analog Output file //changed strAO_XY_FileName => strAO_Y_FileName

	/*********************************************/	
	// Load Analog Output Buffer
	/*********************************************/
	ConstructAOBuffer(&dAOBuffer[0], AOOneChanBufSiz, AOFileSize, AO_X_FileSize, ptrAOFile,reinterpret_cast<double*> (ptrAO_X_File), reinterpret_cast<double*> (ptrAO_Y_File)); //The first time this should load the full buffer; changed ptrAO_i_File to ptrAO_i_File >> d, changing extracting doubles from ifstreams for last two arguments in fn. 10/30/2017 [AWT] added a reinterpret_cast<double*> on the last two parameters [AWT] [5/4/2018]
	//ConstructAOBuffer() => ConstructAOBuffer_uInt16() 10/30/2017 [AWT]
	...
	while ( !done && !_kbhit() ) //kbhit() => _kbhit() 10/18/2017 [AWT]
	{
		//********************************************
		...
		if(AOBufferSpaceAvail > AOHalfBuf_Siz)
		{
			...
			ConstructAOBuffer(&dAOBuffer[0], AOOneChanBufSiz, AO_X_FileSize, AOFileSize, ptrAOFile, reinterpret_cast<double*> (ptrAO_X_File), reinterpret_cast<double*> (ptrAO_Y_File)); // added a reinterpret_cast<double*> to the last two parameters, which are incompatible ifstreams [AWT][5/4/2018]
